  ...
  class Validate {
            ...
            switch($rule) {
              case 'min':
                if(strlen($value) < $rule_value) {
                  $this->addError(["{$display} must be a minimum length of {$rule_value} characters.",$item]);
                }
                break;
              case 'max':
                if(strlen($value) > $rule_value) {
                  $this->addError(["{$display} must be a maximum length of {$rule_value} characters.",$item]);
                }
                break;
              case 'matches':
    ...
    public function displayErrors() {
      ...
      if(count($this->_errors)>0) {
        $html = "<ul class='alert alert-danger errorsList'>";
        foreach($this->_errors as $error) {
          if(is_array($error)) {
            $html .= "<li class='text-danger'><strong>Error</strong> ".$error[0]."</li>";
            $html .= '<script>jQuery("document").ready(function(){jQuery("#'.$error[1].'").parent().closest("div").addClass("has-error");});</script>';
          } else {
            $html .= "<li class='text-danger'><strong>Error</strong>".$error."</li>";
          }
        }
        $html .= "</ul>";
      }
